...
`scripts/seed_demo.py` fills the database with realistic demo data (users, file trees, chat, calendars, avatars) and always creates a deterministic login on top of the faker users.
...
```powershell
uv run python scripts/seed_demo.py --users 3 --min-files 2 --max-files 5 --min-messages 2 --max-messages 5 --min-events 1 --max-events 3 --seed 42
```
...
| `--no-files` / `--no-chat` / `--no-calendar` | Skip a generator |
...
- The `demo` user gets files, calendars and conversations like everyone else, so its account looks lived-in.
