...
**Scope:** applies to the `files` module and every module whose UI manipulates files (notes, mail attachments, chat attachments, etc.). If a module manipulates another kind of entity with its own action registry (not files), follow the same principle against that module's equivalent endpoint - `POST /api/v1/projects/actions` for projects and tasks, `POST /api/v1/vault/actions` for vault entries. The vault one differs on a point worth knowing before you copy it: it never answers 404, because a 404 there would confirm that an entry exists in a vault the caller cannot open. Every submitted UUID comes back with a key, and an unreachable one comes back with an empty list.
