...
| `AI_TOOL_CONCURRENCY` | How many independent tool calls one round may run at once (default 4). A bot asked for four pages waits for the slowest fetch instead of the sum of four, and a fifth page starts once one of them is done; tools that write, send or schedule are never batched, whatever this is set to - image generation is the one paid call that is, since several images asked for in one breath are the longest wait a bot has. `1` runs every call one after another. Each parallel call can take a database connection of its own while it runs, and as many image generations can hit your image backend at once, so raise it with the connection pool and that backend's rate limit in mind. |
