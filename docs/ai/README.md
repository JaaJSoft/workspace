...
| `AI_TOOL_CONCURRENCY` | How many read-only tool calls one round may run at once (default 4). A bot asked for four pages waits for the slowest fetch instead of the sum of four, and a fifth page starts once one of them is done; tools that write, send, schedule or bill are never batched, whatever this is set to. `1` runs every call one after another. Each parallel call can take a database connection of its own while it runs, so raise it with the connection pool in mind. |
