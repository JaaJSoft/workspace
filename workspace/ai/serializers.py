...
class AITaskSerializer(serializers.ModelSerializer):
    ...
    def get_result_html(self, obj):
        ...
        if obj.task_type == AITask.TaskType.EDITOR and obj.input_data.get("action") in (
            "explain",
            "summarize",
        ):
            return _md(obj.result)
...
class SummarizeRequestSerializer(serializers.Serializer):
    message_id = serializers.UUIDField(
        help_text="UUID of the mail message to summarize."
    )
...
class ComposeRequestSerializer(serializers.Serializer):
    ...
    context = serializers.CharField(
        required=False, default="", help_text="Optional context."
    )
    account_id = serializers.UUIDField(
        required=False, help_text="Mail account for tone matching."
    )


class ReplyRequestSerializer(serializers.Serializer):
    message_id = serializers.UUIDField(
        help_text="UUID of the mail message to reply to."
    )
...
class EditorActionRequestSerializer(serializers.Serializer):
    ...
    action = serializers.ChoiceField(
        choices=ACTION_CHOICES, help_text="AI action to perform."
    )
    content = serializers.CharField(help_text="Text/code content to process.")
    language = serializers.CharField(
        required=False, default="", help_text="Programming language."
    )
    filename = serializers.CharField(
        required=False, default="", help_text="Filename for context."
    )
    instructions = serializers.CharField(
        required=False,
        default="",
        help_text="Custom instructions (required for custom action).",
    )
