"""Scheduled bot message generation (body of ``ai.generate_scheduled_response``).

Runs the same tool-loop pipeline as ``chat_response.generate_response`` but
prepends a ``## Scheduled action`` instruction to the system prompt so the
bot knows it is acting on a previously scheduled prompt rather than
responding to a new user message.
"""
...
def generate_scheduled(schedule_id: str) -> dict:
