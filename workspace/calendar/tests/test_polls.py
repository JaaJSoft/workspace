...
class PollCRUDTests(PollTestMixin, APITestCase):
    ...
    def test_update_poll_add_slot_preserves_votes(self):
        ...
        self.assertEqual(
            PollVote.objects.filter(slot=slot1, user=self.voter).count(), 1
        )
        self.assertEqual(
            PollVote.objects.filter(slot=slot2, user=self.voter).count(), 1
        )
...
class PollInviteTests(PollTestMixin, APITestCase):
    ...
    def test_invite_excludes_creator(self):
        ...
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertFalse(
            PollInvitee.objects.filter(poll=self.poll, user=self.owner).exists()
        )
    ...
    def test_invite_duplicate_is_idempotent(self):
        ...
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(
            PollInvitee.objects.filter(poll=self.poll, user=self.voter).count(), 1
        )
    ...
    def test_remove_invitee_only_deletes_votes_for_that_poll(self):
        ...
        self.assertTrue(
            PollVote.objects.filter(slot=other_slot, user=self.voter).exists()
        )
        # Vote on this poll should be gone
        self.assertFalse(
            PollVote.objects.filter(slot=self.slot1, user=self.voter).exists()
        )
