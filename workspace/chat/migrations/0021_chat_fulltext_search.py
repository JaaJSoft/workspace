...
# Full-text index over chat message bodies. SQL generated by
# `manage.py fts_sql workspace.chat.services.message_search.CHAT_FTS` and
# pasted, per the migration freezing rule (migrations never import live
# declarations). left(body, 100000) on PG: a generated tsvector over ~1MB
# fails the INSERT itself, so an oversized pasted message must be truncated
# rather than break message sending.
