...
class ChatActivityProviderTests(TestCase):
    def setUp(self):
        ...
        ConversationMember.objects.create(
            conversation=self.alice_group1,
            user=self.alice,
        )
        ConversationMember.objects.create(
            conversation=self.alice_group2,
            user=self.alice,
        )
        ConversationMember.objects.create(
            conversation=self.bob_group,
            user=self.bob,
        )
        ...
        Message.objects.create(
            conversation=self.alice_group1,
            author=self.alice,
            body="Hello from Alice in group 1",
        )
        Message.objects.create(
            conversation=self.alice_group1,
            author=self.alice,
            body="Another msg from Alice in group 1",
        )
        Message.objects.create(
            conversation=self.alice_group2,
            author=self.alice,
            body="Hello from Alice in group 2",
        )
        Message.objects.create(
            conversation=self.bob_group,
            author=self.bob,
