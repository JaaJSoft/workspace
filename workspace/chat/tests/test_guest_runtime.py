...
from workspace.chat.services.meetings import create_meeting, remove_guest, set_locked
...
class GuestRuntimeTests(TestCase):
    ...
    def test_join_without_an_active_call_is_404(self):
        guest, token = self._admit()
        resp = self._post("join", token)
    ...
    def test_join_refused_when_full(self):
        self.other_host = User.objects.create_user(username="host2", password="x")
        calls.start_or_join_call(self.owner, self.meeting.conversation_id)
        guest, token = self._admit()
    ...
    def test_heartbeat_without_an_active_call_is_404(self):
        guest, token = self._admit()
        resp = self._post("heartbeat", token)
        self.assertEqual(resp.status_code, 404)
