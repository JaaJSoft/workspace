...
class AddMembersTests(ChatTestMixin, APITestCase):
    ...
    def test_unauthenticated_rejected(self):
        resp = self.client.post(
            self.url(self.group.uuid), {"user_ids": [self.extra_user.id]}
        )
    ...
    def test_outsider_cannot_add_members(self):
        ...
        resp = self.client.post(
            self.url(self.group.uuid), {"user_ids": [self.extra_user.id]}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_cannot_add_to_dm(self):
        self.client.force_authenticate(self.creator)
        resp = self.client.post(
            self.url(self.dm.uuid), {"user_ids": [self.extra_user.id]}, format="json"
        )
    ...
    def test_invalid_user_ids_rejected(self):
        self.client.force_authenticate(self.creator)
        resp = self.client.post(
            self.url(self.group.uuid), {"user_ids": [99999]}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_empty_user_ids_rejected(self):
        self.client.force_authenticate(self.creator)
        resp = self.client.post(
            self.url(self.group.uuid), {"user_ids": []}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_adding_existing_member_is_idempotent(self):
        self.client.force_authenticate(self.creator)
        resp = self.client.post(
            self.url(self.group.uuid), {"user_ids": [self.member.id]}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
...
class RenameConversationTests(ChatTestMixin, APITestCase):
    ...
    def test_outsider_cannot_rename(self):
        ...
        resp = self.client.patch(
            self.url(self.group.uuid), {"title": "New"}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_member_can_rename_group(self):
        self.client.force_authenticate(self.member)
        resp = self.client.patch(
            self.url(self.group.uuid), {"title": "Renamed"}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_cannot_rename_dm(self):
        self.client.force_authenticate(self.creator)
        resp = self.client.patch(
            self.url(self.dm.uuid), {"title": "New"}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_empty_title_rejected(self):
        self.client.force_authenticate(self.creator)
        resp = self.client.patch(
            self.url(self.group.uuid), {"title": "  "}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
...
class ConversationDescriptionTests(ChatTestMixin, APITestCase):
    ...
    def test_member_can_set_description_on_dm(self):
        self.client.force_authenticate(self.creator)
        resp = self.client.patch(
            self.url(self.dm.uuid), {"description": "DM description"}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_description_can_be_cleared(self):
        ...
        self.client.force_authenticate(self.member)
        resp = self.client.patch(
            self.url(self.group.uuid), {"description": ""}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.group.refresh_from_db()
    ...
    def test_outsider_cannot_update_description(self):
        self.client.force_authenticate(self.outsider)
        resp = self.client.patch(
            self.url(self.group.uuid), {"description": "Nope"}, format="json"
        )
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)
