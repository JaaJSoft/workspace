...
class ConversationStatsTests(ChatTestMixin, APITestCase):
    ...
    def test_stats_with_messages_and_reactions(self):
        ...
        per_member = {
            e["username"]: e["count"] for e in resp.data["messages_per_member"]
        }
...
class UnreadCountModelTests(ChatTestMixin, APITestCase):
    ...
    def test_send_message_increments_other_members(self):
        ...
        self.client.force_authenticate(self.creator)
        self.client.post(
            self._msg_url(self.group.uuid), {"body": "hello"}, format="json"
        )
    ...
    def test_send_message_does_not_increment_author(self):
        ...
        self.client.force_authenticate(self.creator)
        self.client.post(
            self._msg_url(self.group.uuid), {"body": "msg1"}, format="json"
        )
        self.client.post(
            self._msg_url(self.group.uuid), {"body": "msg2"}, format="json"
        )
    ...
    def test_send_message_increments_cumulatively(self):
        ...
        for i in range(5):
            self.client.post(
                self._msg_url(self.group.uuid), {"body": f"msg {i}"}, format="json"
            )
    ...
    def test_delete_message_decrements_unread(self):
        ...
        resp = self.client.post(
            self._msg_url(self.group.uuid), {"body": "to delete"}, format="json"
        )
    ...
    def test_delete_one_of_multiple_unread(self):
        ...
        self.client.post(
            self._msg_url(self.group.uuid), {"body": "msg3"}, format="json"
        )
    ...
    def test_three_members_independent_counts(self):
        ...
        self.client.post(
            self._msg_url(self.group.uuid), {"body": "reply"}, format="json"
        )
