...
class MetricsTests(TestCase):
    ...
    def test_poll_provider_observes_duration_and_increments_events(self):
        ...
        chunks = list(views_sse._poll_provider('calendar', provider, None, self.user.id))
        ...
        self.assertEqual(_sample('sse_events_emitted_total', ev_labels) - before_events, 1)
    ...
    def test_pubsub_stream_increments_pubsub_messages_on_real_message(self):
        ...
        with patch.object(views_sse, '_MAX_CONNECTION_SECONDS', 1), \
                ...
                patch('workspace.core.views_sse.time.monotonic', side_effect=lambda: next(times)):
            list(views_sse._event_stream_pubsub(self.request, fake_redis))
