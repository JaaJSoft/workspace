...
def hex_to_tokens(apps, schema_editor):
    ...
    for hex_color, token in reverse.items():
        Tag.objects.filter(color=hex_color).update(color=token)
    Tag.objects.filter(color="").update(color="ghost")
