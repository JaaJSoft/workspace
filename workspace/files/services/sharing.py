...
def share_file(file_obj, *, target_user, permission, acting_user):
    ...
    if created:
        record_event(file_obj, acting_user, FileEvent.Action.SHARED, {
    ...
    if share.permission != permission:
        ...
        record_event(file_obj, acting_user, FileEvent.Action.SHARE_PERMISSION_CHANGED, {
            'shared_with_id': target_user.pk,
            'shared_with_username': target_user.username,
...
def unshare_file(file_obj, *, target_user, acting_user):
    ...
    if deleted:
        record_event(file_obj, acting_user, FileEvent.Action.UNSHARED, {
            'shared_with_id': target_user.pk,
            'shared_with_username': target_user.username,
        })
    return deleted
...
def create_share_link(file_obj, *, acting_user, password='', expires_at=None):
    ...
    record_event(file_obj, acting_user, FileEvent.Action.LINK_CREATED, {
...
def revoke_share_link(file_obj, *, link_uuid, acting_user):
    ...
    if deleted:
        record_event(file_obj, acting_user, FileEvent.Action.LINK_REVOKED, {
