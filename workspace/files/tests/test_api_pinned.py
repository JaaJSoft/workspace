...
class FavoriteAPITests(APITestCase):
    ...
    def test_add_favorite(self):
        ...
        file = File.objects.create(
            owner=self.user, name="favme.txt", node_type=File.NodeType.FILE
        )
        ...
        self.assertTrue(
            FileFavorite.objects.filter(owner=self.user, file=file).exists()
        )

    def test_remove_favorite(self):
        """Test removing a file from favorites."""
        file = File.objects.create(
            owner=self.user, name="unfavme.txt", node_type=File.NodeType.FILE
        )
    ...
    def test_list_favorites(self):
        ...
        file1 = File.objects.create(
            owner=self.user, name="fav1.txt", node_type=File.NodeType.FILE
        )
        File.objects.create(
            owner=self.user, name="notfav.txt", node_type=File.NodeType.FILE
        )
...
class PinnedFolderAPITests(APITestCase):
    ...
    def test_pin_folder(self):
        ...
        folder = File.objects.create(
            owner=self.user, name="PinMe", node_type=File.NodeType.FOLDER
        )
        ...
        self.assertTrue(
            PinnedFolder.objects.filter(owner=self.user, folder=folder).exists()
        )

    def test_unpin_folder(self):
        """Test unpinning a folder."""
        folder = File.objects.create(
            owner=self.user, name="UnpinMe", node_type=File.NodeType.FOLDER
        )
    ...
    def test_cannot_pin_file(self):
        ...
        file = File.objects.create(
            owner=self.user, name="notafolder.txt", node_type=File.NodeType.FILE
        )
    ...
    def test_list_pinned_folders(self):
        ...
        folder1 = File.objects.create(
            owner=self.user, name="Pinned1", node_type=File.NodeType.FOLDER
        )
        File.objects.create(
            owner=self.user, name="NotPinned", node_type=File.NodeType.FOLDER
        )
    ...
    def test_reorder_pinned_folders(self):
        ...
        folder1 = File.objects.create(
            owner=self.user, name="First", node_type=File.NodeType.FOLDER
        )
        folder2 = File.objects.create(
            owner=self.user, name="Second", node_type=File.NodeType.FOLDER
        )
        PinnedFolder.objects.create(owner=self.user, folder=folder1, position=0)
