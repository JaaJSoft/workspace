...
def _build_context(request, folder=None, is_trash_view=False):
    ...
    VIEW_META = {
        ...
        "trash": (
            ...
            "/files/trash",
            "Trash is empty",
            "Items you delete stay here for a while.",
        ),
        "favorites": (
            "Favorites",
            "/files?favorites=1",
    ...
    group_folders = (
        FileService.user_group_files_qs(request.user)
        .filter(
            parent__isnull=True,
            node_type=File.NodeType.FOLDER,
        )
    ...
    available_groups = request.user.groups.exclude(id__in=groups_with_folders).order_by(
        Lower("name")
    )
...
def properties(request, uuid):
    ...
    if file_obj.node_type == File.NodeType.FOLDER:
        is_pinned = PinnedFolder.objects.filter(
            owner=request.user, folder=file_obj
        ).exists()
...
def events(request, uuid):
    ...
    return render(
        request,
        "files/ui/partials/_events_list.html",
        {
            "file": file_obj,
...
def pinned_folders(request):
    ...
    return render(
        request,
        "files/ui/partials/pinned_folders.html",
        {
...
def group_folders_sidebar(request):
    ...
    return render(
        request,
        "files/ui/partials/group_folders_section.html",
        {
            "group_folders": group_folders,
            "available_groups": available_groups,
        },
...
def view_file(request, uuid):
    ...
    file_obj = (
        File.objects.select_related("locked_by")
    ...
    if (
        user_can_edit
        and file_obj.is_locked()
        and file_obj.locked_by_id != request.user.pk
    ):
        lock_info = {
