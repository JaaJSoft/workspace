...
class MailActivityProvider(ActivityProvider):
    ...
    def _owner_filter(self, user_id):
        """Return Q filter restricting to mail owned by user."""
        if user_id is not None:
            return Q(account__owner_id=user_id)
        return Q()
    ...
    def get_daily_counts(self, user_id, date_from, date_to, *, viewer_id=None):
        ...
        ).filter(self._owner_filter(user_id))
    ...
    def get_recent_events(self, user_id, limit=10, offset=0, *, viewer_id=None):
        ...
        for msg in qs:
            from_name = ''
            if msg.from_address:
                from_name = msg.from_address.get('name') or msg.from_address.get('address', '')
            events.append({
                'icon': 'mail',
                ...
                'actor': {
                    ...
                    'full_name': from_name or msg.account.owner.get_full_name(),
                },
            })
        return events
    ...
    def get_stats(self, user_id, *, viewer_id=None):
        ...
        msg_qs = MailMessage.objects.filter(
            deleted_at__isnull=True,
        ).filter(self._owner_filter(user_id))
