...
def _add_label(action: AddLabelAction, message: MailMessage) -> dict:
    try:
        label = MailLabel.objects.get(
            uuid=action.label_id, account_id=message.account_id
        )
...
def _flag(
    message: MailMessage, db_field: str, db_value: bool, imap_fn, type_: str
) -> dict:
    ...
    try:
        ...
        logger.warning(
            "IMAP %s failed for message %s: %s", type_, message.uuid, scrub(e)
        )
...
def _delete(action: DeleteAction, message: MailMessage) -> dict:
    ...
    # Not redundant on the sync path: apply_rule_to_folder runs this same
