...
class LeafCondition(BaseModel):
    ...
    def _check(self):
        ...
        if self.field in BOOL_FIELDS and self.op not in BOOL_OPS:
            raise ValueError(
                f"op {self.op!r} not valid on boolean field {self.field!r}"
            )
        ...
        if self.op in TEXT_OPS:
            ...
            try:
                ...
                raise ValueError(
                    f"op {self.op!r} value is not a valid ISO 8601 date or datetime"
                ) from None
        return self
...
ConditionNode = Annotated[
    Annotated[LeafCondition, Tag("leaf")] | Annotated[GroupCondition, Tag("group")],
...
def parse_conditions(payload: Any) -> LeafCondition | GroupCondition:
...
def validate_tree_limits(node: LeafCondition | GroupCondition) -> None:
