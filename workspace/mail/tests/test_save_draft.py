...
class SaveDraftTests(TestCase):
    ...
    def test_append_success_then_deletes_old_draft(self, mock_connect, _sync):
        ...
        mock_connect.return_value = conn
        call_order = []
        conn.append.side_effect = lambda *a, **kw: call_order.append('append') or ('OK', [b''])
        conn.uid.side_effect = lambda *a, **kw: call_order.append(('uid',) + a) or ('OK', [b''])
        conn.expunge.side_effect = lambda *a, **kw: call_order.append('expunge') or ('OK', [b''])
