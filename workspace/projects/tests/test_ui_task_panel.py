...
class TaskPanelViewTests(SettingsCleanupMixin, ProjectTestMixin, TestCase):
    ...
    def test_archived_project_renders_read_only(self):
        ...
        # control must be disabled: the status, priority, epic, start-date
        # and due-date fields and the checklist item checkbox. Assignees and
        # labels render as chips whose remove controls and selectors are
        # omitted entirely, so they carry no disabled attribute.
        self.assertEqual(self._count_disabled(resp), 6)
