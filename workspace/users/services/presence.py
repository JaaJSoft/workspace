...
def get_manual_status(user_id: int) -> str:
    ...
    try:
        ms = UserPresence.objects.values_list("manual_status", flat=True).get(
            user_id=user_id
        )
...
def get_statuses(user_ids: list[int]) -> dict[int, str]:
    ...
    if missing_manual:
        ...
        db_vals = dict(
            UserPresence.objects.filter(user_id__in=missing_manual).values_list(
                "user_id", "manual_status"
            )
        )
        to_cache = {}
    ...
    return list(
        UserPresence.objects.filter(
            Q(
                ...
                manual_status__in=("auto", "online", "busy", "away"),
            )
...
def get_last_seen(user_id: int) -> datetime | None:
    ...
    try:
        return UserPresence.objects.values_list("last_seen", flat=True).get(
            user_id=user_id
        )
    except UserPresence.DoesNotExist:
        return None
